        ...
        if($busbnf>1){    array_push($array,$con+3); array_push($arraytd,1); //Ingreso 1 como inicio de td
          foreach($myhtml->find("a") as $content2){ //for para iterar todos los registros
            ...
            if($nv<$fin){ 
              ...
              $casa2=$data[$nv+3]->innertext;   $eve2=$data[$nv+4]->innertext;    $cu2=$data[$nv+5]->innertext;     
              $feve=$datatd[$td+2]->plaintext;  $benf=$datatd[$td]->plaintext;    $eve3valid=$data[$nv+7]->innertext;settype($eve3valid,"string"); 
              $mer1=$datatd[$td+4]->plaintext;  $mer2=$datatd[$td+10]->plaintext; $mer3=$datatd[$td+16]->plaintext;  
              $cuteve1=substr($eve1,0,6);       $cuteve2=substr($eve2,0,6);       $cuteve3=substr($eve3valid,0,6);
              if($cuteve1==$cuteve3 or $cuteve2==$cuteve3 ){
              ...
              $insert ="INSERT INTO `bet`(`IdBet`, `Deporte`, `Competicion`, `Evento`, `FechaEv`, `Mercado1`, `Mercado2`, `Mercado3`, `Cuota1`, `Cuota2`, `Cuota3`, `Casa1`, `Casa2`, `Casa3`, `Beneficio`, `Resultado`, `Limite`, `FechaReg`)
                          VALUES (null,'Futbol',null,'$eve2','$feve','$mer1','$mer2','$mer3',$cu1,$cu2,$cu3,'$casa1','$casa2','$casa3','$benf',101,1500,CURRENT_TIME)";
